...
/*\
 * Verify that :manpage:`profil(3)` collects execution profiling data.
 *
 * Set up a profiling buffer, turn profiling on, spin the PC for a fixed
 * amount of CPU time, then check that the buffer recorded some samples.
 */
...
static void alrm_handler(int sig LTP_ATTRIBUTE_UNUSED)
{
	profil_done = 1;
}
...
static void __attribute__((noinline)) *get_pc(void)
{
	 ...
	 */
	return (void *)((unsigned long)__builtin_return_address(0) & 0x7fffffffUL);
#else
...
static void run(void)
{
	...
	if (ret)
		tst_brk(TBROK, "profil returned: %d", ret);

	SAFE_SIGNAL(SIGALRM, alrm_handler);
	alarm(PROFIL_TIME);
	...
	for (i = 0; i < PROFIL_BUFLEN; i++) {
		if (buf[i]) {
			tst_res(TINFO, "buf[0x%04x]=%d", i, buf[i]);
			count += buf[i];
		}
	}

	if (count > 0)
		tst_res(TPASS, "profil recorded some data");
	else
		tst_res(TFAIL, "profil failed to record anything");
}

static struct tst_test test = {
	.test_all = run,
	.timeout = PROFIL_TIME + 30,
};

#else
TST_TEST_TCONF("system doesn't have profil() support");
#endif
