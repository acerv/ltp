...
static void apply_memory_pressure(void)
{
	...
	while (swapped <= MEM_CHILD) {
        ...
        SAFE_CG_SCANF(tst_cg, "memory.swap.current", "%zu", &swapped);
	}
}
...
static size_t check_normal(void)
{
	size_t swap;
	size_t swap_after;
	size_t swap_before;
    ...
    SAFE_CG_SCANF(tst_cg, "memory.swap.current", "%zu", &swap_before);

	TST_CHECKPOINT_WAKE_AND_WAIT(0);

    SAFE_CG_SCANF(tst_cg, "memory.swap.current", "%zu", &swap_after);

	swap = swap_after - swap_before;

	TST_CHECKPOINT_WAKE(0);

	tst_res(TINFO, "Normal advise swapped %zu bytes", swap);

	return swap;
}

static size_t check_cold(int pid)
{
	int ret;
	int pidfd;
	size_t swap;
	...
	tst_res(TINFO, "MADV_COLD advise swapped %zu bytes", swap);
...
static void setup(void)
{
	...
	SAFE_CG_PRINTF(cg_mem, "memory.high", "%d", MEM_LIMIT);
